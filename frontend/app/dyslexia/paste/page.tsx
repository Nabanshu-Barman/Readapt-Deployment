...
export default function DyslexiaPastePage() {
  ...
  const proceed = () => {
    // Persist text and dyslexia context
    localStorage.setItem("readapt:text", text)
    localStorage.setItem("readapt:lastMode", "dyslexia")

    // Map label (0 severe, 1 mild, 2 none) -> preset (1..3)
    const label = Number(localStorage.getItem("readapt:label") ?? "1")
    const dyslexiaPreset = label === 2 ? 1 : label === 1 ? 2 : 3
    localStorage.setItem("readapt:dyslexiaPreset", String(dyslexiaPreset))

    // Always go to dyslexia adapt from this page
    ...
    <main className="relative min-h-dvh">
      ...
      <section className="mx-auto max-w-4xl px-4 md:px-6 pt-8 pb-16">
        <div className="glass rounded-2xl p-6 grid gap-4">
          <h1 className="text-2xl font-semibold">Paste your text (Dyslexia)</h1>
          <Textarea
            value={text}
