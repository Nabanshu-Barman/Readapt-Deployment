...
import {
  defaultLowVisionCustomSettings,
  saveLowVisionCustomSettings,
 ...
 */
const PREVIEW = "Low vision adaptive preview text.\nAdjust fonts, spacing and contrast for comfort.";
...
/**
 * Intent heuristic override (identical to dyslexia custom)
 */
...
export default function LowVisionCustomPage() {
  const [mounted, setMounted] = useState(false);
  const [settings, setSettings] = useState(() => ({
    ...defaultLowVisionCustomSettings,
    fontFamily: FONT_FAMILIES[defaultLowVisionCustomSettings.fontFamilyIndex],
  }));
  ...
  useEffect(() => {
    if (mounted) {
      loadLocalIntentModel().catch(e =>
        console.error("[LowVisionCustom] model preload error:", e)
      );
    }
  }, [mounted]);
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      try {
        const stored = loadLowVisionCustomSettings();
        if (stored) {
        ...
        console.warn("[LowVisionCustom] failed to load stored settings", e);
      }
    }, 0);
  }, []);
  ...
  const applyIntent = useCallback((intent: string, prev: typeof settings) => {
    ...
    switch (intent) {
      ...
      case "cycle_font_prev":
        next.fontFamilyIndex =
          (next.fontFamilyIndex - 1 + FONT_FAMILIES.length) % FONT_FAMILIES.length;
        break;
      case "increase_contrast":
        next.contrastIndex = (next.contrastIndex + 1) % CONTRAST_PRESETS.length; break;
      case "decrease_contrast":
        next.contrastIndex =
          (next.contrastIndex - 1 + CONTRAST_PRESETS.length) % CONTRAST_PRESETS.length;
        break;
      ...
      default:
        return {
          next,
            message:
            "Try: 'make it bigger', 'contrast change', 'loosen letters', 'more word spacing', 'decrease line spacing'.",
        };
    }
  ...
  async function handleSend() {
    ...
    if (!overrideIntent) {
      try {
        ...
        console.error("[LowVisionCustom] classifyIntentLocal error:", e);
        intent = "other";
      }
    }
  ...
  return (
    ...
    >
      {/* Reuse dyslexia variant header for identical look */}
        ...
        <div className={clsx("backdrop-blur-md rounded-2xl overflow-hidden shadow-xl border",
          ...
          <div className={clsx("px-5 py-3 border-b flex items-center justify-between", theme.border)}>
            <div className="text-sm font-medium tracking-wide flex items-center gap-3">
              <span>Low Vision Customization</span>
              <Button
                size="sm"
                variant="outline"
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  saveLowVisionCustomSettings(settings);
          ...
          <div className="grid md:grid-cols-2 gap-0 md:gap-10 p-6 md:p-10">
            ...
            <div>
              ...
              <div className={clsx("mt-3 text-xs space-y-1", theme.textSoft)}>
                <div>Font: <code>{renderSettings.fontFamily.split(",")[0].replace(/'/g,"")}</code></div>
