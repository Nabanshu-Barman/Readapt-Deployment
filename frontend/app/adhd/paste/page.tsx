...
export default function ADHDPastePage() {
  ...
  const proceed = () => {
    // Persist text and ADHD context
    localStorage.setItem("readapt:text", text)
    localStorage.setItem("readapt:lastMode", "adhd")

    // Map label (class 0-3) -> preset (1..4, can tweak as needed)
    const adhdClass = Number(localStorage.getItem("readapt:adhdClass") ?? "0")
    const adhdPreset = Math.min(4, Math.max(1, adhdClass + 1))
    localStorage.setItem("readapt:adhdPreset", String(adhdPreset))

    // Always go to ADHD adapt from this page
    ...
    <main className="relative min-h-dvh">
      ...
      <section className="mx-auto max-w-4xl px-4 md:px-6 pt-8 pb-16">
        <div className="glass rounded-2xl p-6 grid gap-4">
          <h1 className="text-2xl font-semibold">Paste your text (ADHD)</h1>
          <Textarea
            value={text}
